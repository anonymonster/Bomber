...
pause() {
    read -n1 -r -p "Press any key to continue...?!Monster the best" key
}
banner() {
    clear
    echo -e "\e[1;31m"
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing MONSTER'
    else
        figlet MONSTER
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mMonster \e[0m"
    else
...
do
    ...
    echo ""
    echo "Don't forgot Monster The best !!!"
    echo "Press 1 To  Start SMS  Bomber "
    echo "Press 2 To  Start CALL Bomber "
    echo "Press 3 To  Start MAIL Bomber (jare bar dast nya )"
    echo "Press 4 To  Update (lasar Linux & Linux Emulators esh akat) "
    echo "Press 5 To  Exit & chwna darawa "
    read ch
    clear
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[1;34m RUN MONSTER Again..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
